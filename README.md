...
## 维护分支 / Maintenance fork

本仓库是 [zh667/TokenLedger](https://github.com/zh667/TokenLedger)（`dsh-tokenledger@0.1.0`，MIT）
的维护分叉 **v0.1.1**，在上游基础上修复了与当前 DeepSeek Harness 的兼容性断裂，并补齐了日常看板最需要的两项默认值：

| 变更 | 说明 |
| --- | --- |
| 🔧 **会话持久化 API 修复** | Harness 2026-08-28 起把 `sessionPersistence` 换成了 handle-based 模型（`bec6805d6a`），`listSnapshots()` / `readFrom(id, seq)` 已删除。原版每次扫描第一步就抛 `TypeError`，异常静默吞掉 → 面板永远为空。本分支改走 `list()` / `open(id, "read")` → `read(offset)` → `close()`，同时保留旧接口回退分支。 |
| 🔢 **seq 重编号兜底** | 会话 v1→v2 迁移会把事件 `seq` 重新编号，旧 checkpoint 的 `consumedSeq` 续读会越过整个日志。本分支在 revision 变化时从 0 全量重折叠（`commitSession` 按会话整表替换，重折叠幂等，不会重复计数）。 |
| 💰 **官方 ¥ 价目表兜底** | 未配置 `rates` 时自动按 DeepSeek 官方人民币价（峰时）计价：`deepseek-v4-flash` / `-vision-exp` ¥0.10/M 命中、¥3/M 未命中、¥9/M 输出；`deepseek-v4-pro` ¥0.30/M、¥9/M、¥27/M（源：<https://api-docs.deepseek.com/zh-cn/quick_start/pricing/>，2026-09-08 抓取）。徽章显示「今日总 token · 今日金额」，面板用量详情的金额列与汇总同时生效；非 DeepSeek 模型仍保持「—」不瞎算。配置了 `rates` 时永远优先你的价目表。 |
| 🔤 **Token 单位紧凑化** | 徽章、三窗口卡片、站点/模型行、活跃度热力图全部改用 `1.33B / 27.69M / 862.4K` 展示，超大数字不再溢出。 |

> 与上游 npm 包同名（`dsh-tokenledger`）。如需把它作为插件安装：
