...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function TokenLedgerPanel({ wide, t }) {
			...
			const badgeTokens = todayWindow === undefined ? "" : fmtCompact(todayWindow.tokens);
			const badgeCost = todayCostAmount === null || todayCostAmount === undefined
				? ""
				: fmtMoney(todayCostAmount.cost, todayCostAmount.currency);
			const badgeLabel = badgeTokens === ""
				? ""
				: [badgeTokens, badgeCost].filter(Boolean).join(" · ");
				...
				children: [
					jsxs("button", {
						...
						children: [
							...
							jsx("span", { className: S.badgeValue, children: badgeLabel })
						]
					}),
					open &&
						jsxs("div", {
