 ...
 * parsing it, and a handle opened with `open(id, "read")` reads the log.
 * Subscribing to live events instead would put this code on the hot path and
 * lose everything written while the plugin was not running — a restart would
 * silently under-count. Sweeping is idempotent and self-healing; subscribing
 * is neither.
 *
 * @module dsh-tokenledger/plugin
 */

import { applyUsageDelta, createUsageState, dayKey } from "./usage.js";
...
import { registerRoutes, priceToday } from "./http.js";
 ...
 *
 * Rates live in config when a relay sets its own prices — shipping a table
 * would be shipping one site's deal as everyone's. But the official route is
 * the default install, and a report whose cost column is all em dashes until
 * the user writes a price table reads as broken. The official list prices the
 * deepseek models only; anything else stays unpriced (`null`, never zero).
 */
...
export async function sweep(persistence, store, options = {}) {
	...
	let snapshots;
	try {
		// The harness reworked this seam on 2026-08-28 (handle-based session
		// persistence): `listSnapshots()` became `list()`. Prefer the new name,
		// keep the old one as a fallback so this package still runs on older
		// hosts.
	...
	for (const snapshot of snapshots ?? []) {
		...
		try {
			...
			// The handle-based seam reads through `open(id, "read") → read(offset)`,
			// replacing `readFrom(id, seq)`. The v1→v2 session migration renumbers
			// event seqs, so a checkpoint's `consumedSeq` can silently overshoot
			// the new log and yield an empty tail while unread events remain. The
			// fold therefore always re-reads from 0 whenever the revision moved;
			// `commitSession` replaces the session's rows wholesale, so re-folding
			// the same events cannot double count.
			let events;
			if (typeof persistence.open === "function") {
				...
				({ events } = await persistence.readFrom(sessionId, 0));
			}
...
export function apply(ctx, userConfig = {}) {
	...
	try {
		const served = registerRoutes(ctx, {
			...
			sites: () => directory.sites,
			sweep: runSweep,
			priced: (range, site) => priceWithConfiguredRates(store, range, site, config.rates),
			// The badge's "today" figure: the configured rates when present, the
			// shipped DeepSeek official list otherwise — same default as `priced`.
